...
# 要約(summary)の書き方ナレッジ。update_videos.py とも共有する。
SUMMARY_GUIDE = """
## summary（要約）の書き方【最重要ルール】
- タイトルの言い換えは禁止。字幕から読み取れる「具体的な内容・手法・ポイント」を書く
- **当たり前の前置きを書かない**（読み手は「よすがのFXチャンネル」だと分かっている）。
  次の語句で始めない・入れない: 「よすが」「よすが氏」「プロトレーダーが」「FX教育チャンネル」
  「この動画は」「本動画は」「〜という動画」。主語・肩書き・媒体名は省き、いきなり内容から書く
- 冗長にしない。**1文・日本語60文字前後（最大80文字）**。読むのに疲れない簡潔さを最優先
- 文体は体言止め「〜を解説している」「〜を紹介している」
- 良い例:「損切りラインの決め方3パターンと、エントリー後のメンタル管理を解説している。」
- 悪い例:「FX教育チャンネルのよすが氏が、プロトレーダーの視点から損切りについて詳しく解説している動画。」"""
...
def generate_metadata(title, transcript, need_full):
    ...
    summary_instruction = SUMMARY_GUIDE
